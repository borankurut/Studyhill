      ...
      if (u.hasGroup) {
        Group.findGroup(u.groupCode, function cb(g) {
          ...
          if (findLastMonday(new Date()) != dateToStr(g.mondayDate)) {
            ...
            const sqlUpdMonday =
              `UPDATE snowhill.groups SET mondayDate = "${findLastMonday(
                new Date()
              )}" ` + `WHERE groupCode = "${g.groupCode}"`;

            db.query(sqlUpdMonday, function cb(error, results) {
            ...
            db.query(sqlResetTimes, function cb(error, results) {
              if (error) throw error;
            });
          }
  ...
  db.query(
    ...
    async (error, result) => {
      ...
      User.findUserUsername(username, function cb(u) {
        if (u) return res.json({ alreadyExists: true });
...
app.get("/leaderboard-of", function (req, res) {
  ...
  const sqlLeaderboard = `SELECT username, studyTime FROM snowhill.group_table_code_${groupCode} ORDER BY studyTime DESC;`;
...
app.put("/joingroup", (req, res) => {
  ...
  Group.findGroup(groupCode, function cb(g) {
    if (!g) return res.json({ msg: "Invalid Code" }); // send message if eror.

    if (g.memberCount >= g.maxSize) return res.json({ msg: "Group is full" });
    else {
...
app.put("/creategroup", (req, res) => {
  ...
  if (maxSize < 2 || maxSize > 8) return res.json({ msg: "size is not valid" });
